...
def apply_accept_trade(state: State, action: Action):
    ...
    try:
        ...
        state.current_player_index = next(
            ...
            if c != state.colors[state.current_turn_index] and i > state.current_player_index
        )
...
def yield_resources(board: Board, resource_freqdeck, number):
    ...
    for player, player_payout in intented_payout.items():
        ...
        for resource, count in player_payout.items():
            ...
            elif sum(bool(values.get(resource, 0)) for values in intented_payout.values()) == 1:
                # 2025 rulebook: a sole recipient takes the remaining supply.
                freqdeck_replenish(payout[player], resource_freqdeck[RESOURCES.index(resource)], resource)
