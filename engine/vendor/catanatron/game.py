...
def is_valid_action(playable_actions, state: State, action: Action) -> bool:
    ...
    if action.action_type == ActionType.OFFER_TRADE:
        return (
            ...
            and all(
                state.player_state[f"{player_key(state, action.color)}_{resource}_IN_HAND"] >= count
                for resource, count in zip(RESOURCES, action.value[:5])
            )
        )
...
def is_valid_trade(action_value):
    ...
    """
    if not isinstance(action_value, (tuple, list)) or len(action_value) != 10:
        return False
    if any(type(count) is not int or count < 0 or count > 19 for count in action_value):
        return False
...
class Game:
    ...
    def winning_color(self) -> Union[Color, None]:
        ...
        """
        if self.state.is_initial_build_phase:
            return None
        color = self.state.colors[self.state.current_turn_index]
        key = player_key(self.state, color)
        return color if self.state.player_state[f"{key}_ACTUAL_VICTORY_POINTS"] >= self.vps_to_win else None
    ...
    def copy(self) -> "Game":
        ...
        game_copy.random = random.Random()
        game_copy.random.setstate(self.random.getstate())
        ...
        game_copy.state.random = game_copy.random
