...
COSTS = {'BUILD_ROAD':[1,1,0,0,0], 'BUILD_SETTLEMENT':[1,1,1,1,0], 'BUILD_CITY':[0,0,0,2,3], 'BUY_DEVELOPMENT_CARD':[0,0,1,1,1]}
    ...
    for building in buildings:
        for tile in observation['board']['tiles']:
            if building['node'] in tile['nodes'] and tile['resource']:
                ...
                result[r] += (6-abs(7-tile['number'])) * (2 if building['type']=='CITY' else 1)
    return result

def economy_score(pips):
    # Preference scale, not a win probability. Ore/wheat enable cities and cards.
    weights = [1.0,.9,.8,1.25,1.2]
    return sum(weights[i]*v for i,v in enumerate(pips)) + sum(2.0*math.sqrt(v) for v in pips) + 2*min(pips[3],pips[4])
...
def rank_actions(observation):
    ...
    for action in actions:
        ...
        score={'END_TURN':0,'ROLL':10,'BUY_DEVELOPMENT_CARD':45,'PLAY_KNIGHT_CARD':46,'PLAY_ROAD_BUILDING':60,'PLAY_MONOPOLY':25,'PLAY_YEAR_OF_PLENTY':55,'ACCEPT_TRADE':-1,'REJECT_TRADE':0,'CANCEL_TRADE':0,'CONFIRM_TRADE':20}.get(kind,0)
        if kind=='BUILD_SETTLEMENT':
            ...
            score=95+economy_score(production(observation,color,value))-score_before
            ...
            for node in value: candidates.update(neighbors[node])
            potential=max([economy_score(production(observation,color,n))-score_before for n in candidates if n not in occupied and not any(x in occupied for x in neighbors[n])] or [0])
            # Avoid exhausting roads when settlement sites are unavailable; pursue longest road too.
            score=12 + potential*.6 + (5 if own['roads_available']<9 else 0)
            if own['settlements_available']==0: score=4 if own['roads_available'] else -1
            ...
            tile=next(t for t in observation['board']['tiles'] if t['coordinate']==coordinate)
            score=0
            for building in observation['board']['buildings']:
                if building['node'] in tile['nodes']:
                    points=next(p['public_points'] for p in observation['players'] if p['color']==building['color'])
                    score+=(6-abs(7-(tile['number'] or 7)))*(2 if building['type']=='CITY' else 1)*(-3 if building['color']==color else 1+points/10)
            if victim: score+=2
            reason='Block visible opponent production while avoiding our own settlements.'
        elif kind=='MARITIME_TRADE':
            updated=hand.copy()
            for resource in value[:-1]:
                if resource: updated[RESOURCES.index(resource)]-=1
            updated[RESOURCES.index(value[-1])]+=1
            score=-2
            for target,cost in COSTS.items():
                if target=='BUILD_CITY' and not any(b['color']==color and b['type']=='SETTLEMENT' for b in observation['board']['buildings']):continue
                if target=='BUILD_SETTLEMENT' and not own['settlements_available']:continue
                if target=='BUILD_ROAD' and not own['roads_available']:continue
                before=sum(max(0,c-h) for c,h in zip(cost,hand))
                after=sum(max(0,c-h) for c,h in zip(cost,updated))
                if after<before:score=max(score,(22 if after==0 else 2)+(before-after))
            reason='Trade only when it reduces the resource shortfall for a build.'
        elif kind in ('ACCEPT_TRADE','CONFIRM_TRADE'):
            giving,receiving=(value[5:10],value[:5]) if kind=='ACCEPT_TRADE' else (value[:5],value[5:10])
            weights=[1,1,.8,1.2,1.2]
            ...
            score+=sum(hands.get(p['color'],[{'expected':0}]*5)[r]['expected'] for p in observation['players'] if p['color']!=color)
...
def draft_search(observation, trials=32):
    ...
    Samples softmax opponent choices from public geometry. Evaluates the eventual
    ...
    if observation['phase']!='BUILD_INITIAL_SETTLEMENT':
        raise ValueError('Draft search is available when placing an opening settlement.')
    ...
    for candidate in candidates:
        ...
        for trial in range(trials):
            rng=random.Random(971+trial)  # common scenario randomness, independent of game RNG
            used=set(occupied)|{candidate['value']}
            added={c:[] for c in order}
            added[color].append(candidate['value'])
            for player in remaining:
                feasible=[n for n in sorted(neighbors) if n not in used and not(neighbors[n]&used)]
                if not feasible:break
                ranks=sorted(feasible,key=lambda n:(-economy_score(production(observation,player,n)),n))[:5]
                node=rng.choices(ranks,weights=[5,4,3,2,1][:len(ranks)])[0]
                used.add(node);added[player].append(node)
        ...
        output.append({**candidate,'draft_score':round(mean,2),'standard_error':round(math.sqrt(variance/len(values)),2),'samples':trials})
    return {'method':'public-opening-draft-monte-carlo','objective':'Heuristic resource economy after the draft; not win probability. Opponents use sampled production preferences. Roads and continuation play are not rolled out.','candidates':sorted(output,key=lambda a:(-a['draft_score'],a['id'])),'trials_per_candidate':trials}
