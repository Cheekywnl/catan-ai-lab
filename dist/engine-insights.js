...
export function insights(s, selected, plan, forecast) {
 ...
 <details class="card development-model"><summary>Development-card possibilities</summary><p>${esc(s.development_belief.assumption)}</p><div class="table-scroll" tabindex="0" role="region" aria-label="Development card probabilities"><table><thead><tr><th>Opponent</th><th>Has a knight</th><th>Has a victory point</th><th>Has a monopoly</th><th>Hidden VP range</th></tr></thead><tbody>${Object.entries(
   s.development_belief.hands,
 )
   .map(
     ([color, cards]) =>
       `<tr><th>${title(color)}</th>${["KNIGHT", "VICTORY_POINT", "MONOPOLY"].map((card) => `<td>${percent(cards[card].probability_at_least_one)}</td>`).join("")}<td>${s.development_belief.constraints[color].vp_min}–${s.development_belief.constraints[color].vp_max}</td></tr>`,
   )
   .join(
     "",
   )}</tbody></table></div><p class="micro">A completed turn on nine visible points rules out older victory-point cards. New purchases can change that bound. Probabilities condition on those public constraints; strategic card retention is not modeled.</p></details>`;
}
