...
import { mountEngine } from "./engine-ui.js";
...
let engineCleanup;
const routes = {
  engine: "Play & simulate",
  overview: "Overview",
  research: "Full research",
  projects: "Project landscape",
...
function sources() {
  return `${heading("THE EVIDENCE REGISTER", "Every claim has a trail.", "Papers, repositories, pinned code revisions, and data checks behind the report. Inspected 11 September 2026.", "37 numbered sources")}<div class="filter-bar"><div class="field search-field"><label for="source-query">Search the evidence</label><input type="search" id="source-query" placeholder="Author, topic, project, or source number…" maxlength="200"></div><div class="field"><label for="source-kind">Source type</label><select id="source-kind"><option value="all">All source types</option>${Object.entries(
    ...
    .join(
      "",
    )}</select></div><span class="result-count" id="source-count" role="status">37 sources</span></div><div class="source-list" id="source-results">${data.sources.map(sourceCard).join("")}</div>`;
}
...
function search(query) {
  ...
  const matching = q
    ? data.chapters.filter((c) =>
        (c.title + " " + c.text).toLowerCase().includes(q.toLowerCase()),
      )
    : [];
  const sourceMatches = q
    ? data.sources.filter((s) => s.text.toLowerCase().includes(q.toLowerCase()))
    : [];
  return `${heading("SEARCH THE WORKSPACE", q ? `Results for “${escape(q)}”` : "Find a thread of evidence.", q ? `${matching.length} matching chapters and ${sourceMatches.length} sources.` : "Use the search field above to explore the full report and source register.")}<div class="search-results">${matching
    .map((c) => {
      const at = c.text.toLowerCase().indexOf(q.toLowerCase());
      const start = Math.max(0, at - 90);
      return `<a class="search-result" href="#research/${c.id}"><span class="eyebrow">CHAPTER ${String(c.id).padStart(2, "0")}</span><h2>${escape(c.title)}</h2><p>${start ? "…" : ""}${escape(c.text.slice(start, start + 270))}…</p><span class="arrow-link">Open chapter ↗</span></a>`;
    })
    .join(
      "",
...
function notFound() {
  return `${heading("WORKSPACE", "That page could not be found.", "Choose a section in the navigation, or return to the overview.")}<a class="button" href="#overview">Back to overview</a>`;
}
...
function filterProjects() {
  ...
  const matching = projects.filter(
    (p) =>
      ...
      Object.values(p).flat().join(" ").toLowerCase().includes(q),
  );
  $("#project-results").innerHTML =
    matching.map(projectCard).join("") ||
    '<div class="empty-state"><h2>No projects match.</h2><p>Clear the search or choose another focus.</p></div>';
  $("#project-count").textContent =
    `${matching.length} project${matching.length === 1 ? "" : "s"}`;
}
function filterSources() {
  const q = $("#source-query").value.trim().toLowerCase();
  const kind = $("#source-kind").value;
  const matching = data.sources.filter(
    (s) =>
      (kind === "all" || s.group === kind) &&
      (s.text.toLowerCase().includes(q) || String(s.id) === q),
  );
  $("#source-results").innerHTML =
    matching.map(sourceCard).join("") ||
    '<div class="empty-state"><h2>No sources match.</h2><p>Clear the search or choose another source type.</p></div>';
  $("#source-count").textContent =
    `${matching.length} source${matching.length === 1 ? "" : "s"}`;
  externalLinks();
}
function updateBelief() {
  const oreInput = $("#ore-count"),
    wheatInput = $("#wheat-count");
  const ore = oreInput.valueAsNumber,
    wheat = wheatInput.valueAsNumber;
  const valid =
    [oreInput, wheatInput].every(
      (el) => el.value !== "" && el.validity.valid,
    ) && ore + wheat > 0;
  [oreInput, wheatInput].forEach((el) =>
    el.setAttribute("aria-invalid", String(!valid)),
  );
  if (!valid) {
    $("#belief-outcomes").innerHTML =
      '<p class="input-error" role="alert">Enter whole numbers from 0 to 10, with at least one card in total.</p>';
    $("#known-hand").innerHTML = "";
    return;
  }
...
function externalLinks() {
  ...
  main.querySelectorAll(".prose table").forEach((table) => {
    ...
    wrap.setAttribute(
      "aria-label",
      "Research comparison table, scroll horizontally if needed",
    );
...
function render(focus = false) {
  ...
  let content;
  if (route === "engine") content = '<div id="engine-root"></div>';
  else if (route === "overview") content = overview();
  ...
  main.classList.toggle("engine-main", route === "engine");
  if (route === "engine")
    engineCleanup = mountEngine(document.querySelector("#engine-root"));
  ...
  $("#navigation")
    .querySelectorAll("a")
    .forEach((a) => {
      const active = a.hash.slice(1).split("/")[0] === route;
      a.classList.toggle("active", active);
      if (active) a.setAttribute("aria-current", "page");
      else a.removeAttribute("aria-current");
    });
  if (route === "projects") {
    $("#project-query").addEventListener("input", filterProjects);
    $("#project-kind").addEventListener("change", filterProjects);
  }
  if (route === "sources") {
    $("#source-query").addEventListener("input", filterSources);
    $("#source-kind").addEventListener("change", filterSources);
  }
  if (route === "belief") {
    ["#ore-count", "#wheat-count"].forEach((s) =>
      $(s).addEventListener("input", updateBelief),
    );
    $("#reset-belief").addEventListener("click", () => {
      $("#ore-count").value = 2;
      $("#wheat-count").value = 1;
      updateBelief();
    });
...
$("#global-search").addEventListener("submit", (event) => {
  ...
  const next =
    "#search?q=" + encodeURIComponent($("#global-query").value.trim());
  if (location.hash === next) render(true);
