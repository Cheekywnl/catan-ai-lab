...
export function mountEngine(root) {
  let disposed = false,
    ...
    plan = null,
    forecast = null;
  root.innerHTML = `<div class="page-heading engine-heading"><div><div class="eyebrow">ENGINE 0.4 · FOUR-PLAYER BASE CATAN</div><h1>Play. Inspect. Simulate.</h1><p class="lede">Resource planning, hidden-world search, and turn-by-turn winning forecasts.</p></div><span class="pill green">Tactical policy + search</span></div><div class="engine-toolbar"><div class="field"><label for="game-seed">Game seed</label><input id="game-seed" type="number" min="0" max="4294967295" step="1" value="${seed}"></div><button class="button" id="new-game">New game</button><div class="field"><label for="game-viewer">View as</label><select id="game-viewer">${Object.keys(
    colors,
  )
    .map((c) => `<option ${c === viewer ? "selected" : ""}>${c}</option>`)
    .join(
      "",
    )}</select></div><div class="field"><label for="bot-policy">Bot policy</label><select id="bot-policy"><option value="tactical">Tactical v4 · tracked cards</option><option value="tactical_v3">Frozen tactical v3</option><option value="strategic">Frozen strategic v2</option><option value="baseline">Original baseline</option><option value="search">Experimental rollout search</option></select></div><button class="button" id="undo-game">Undo move</button><button class="button" id="export-game">Export replay ↓</button><label class="button import-replay">Import replay<input id="import-game" type="file" accept=".json,application/json" aria-label="Import replay file"></label></div><p class="engine-status" role="status" id="engine-status">Loading the game engine…</p><div id="engine-position"><div class="engine-loading"><span class="loading-orbit" aria-hidden="true">⬡</span><h2>Preparing the board</h2><p>The engine runs on your device. The first load downloads the Python runtime.</p></div></div><section class="card engine-validation"><span class="eyebrow">VALIDATION · ENGINE 0.4</span><h2>Built to be inspected.</h2><p>Tactical v4 now uses tracked resource cards in autoplay, matching the displayed suggestions. The previous input mismatch affected 43 of 77 Monopoly decisions in a replay audit; all now agree. It won 215/800 games (26.875%) against three frozen Tactical-v3 opponents on fresh boards, with no errors or unfinished games. The board-based 95% interval is 25.25–28.50%, a modest improvement over the 25% equal-policy reference. All 210 native tests and 24 browser checks pass.</p><p>Public completed turns now constrain hidden victory-point cards. A 40-game replay audit found seven contradictory estimates in the previous model; the new model resolves all seven while preserving every replay. The 0.3.2 release passed 203 native tests and 22 browser tests.</p><p>The latest small-budget search trial won 12/100 games versus 29/100 for its matched fast-policy control. It underperformed in that batch; search remains experimental and Tactical v4 is the default. Work is paused at your request. The larger-budget trial is saved at 62/100 games; its result is incomplete.</p><p>Engine 0.3.1 search continuations ran <strong>3.77× faster</strong> in a local six-position benchmark, with identical decisions and results. This measures runtime on the test machine. A separate 400-game trial against the original bots produced 193 wins (48.25%) under the corrected rules.</p><p>Tactical v3 won 222 of 800 games (27.75%) against three frozen strategic-v2 players on 200 fresh boards across all four seats. All games completed without errors. The board-bootstrap 95% interval is 26.13–29.50%, compared with the 25% equal-policy reference. This is evidence against this opponent population, not a GTO or human-strength result. New games also use corrected road blocking and award ties, and sampled hands respect the finite bank.</p><p>The additional Tactical-v4 trials are complete: 223/800 wins against three strategic-v2 bots (27.875%; interval 26–29.75%) and 205/400 against three original bots (51.25%; interval 46.25–56%). All games completed without errors. These opponent populations are reported separately.</p><div class="detail-links"><a href="progress-2026-09-12.md" target="_blank" rel="noopener noreferrer">Progress &amp; resume notes ↗</a><a href="strength-v4-v2.json" download>v4 against v2 ↓</a><a href="strength-v4-original.json" download>v4 against original bots ↓</a><a href="strength-v4.json" download>Tactical v4 validation ↓</a><a href="search12-v032-comparison.json" download>Latest search trial ↓</a><a href="policy-observation-v4.json" download>Autoplay information audit ↓</a><a href="root-search-audit.md" target="_blank" rel="noopener noreferrer">Search repeatability audit ↗</a><a href="development-history-v032.json" download>Hidden-card audit ↓</a><a href="search12-v031-comparison.json" download>Search vs fast-policy trial ↓</a><a href="speed-v3.json" download>Search speed & equivalence ↓</a><a href="strength-v3-original.json" download>Original-bot comparison ↓</a><a href="refinement-log.md" target="_blank" rel="noopener noreferrer">Refinement log ↗</a><a href="strength-v3.json" download>Current opponent benchmark ↓</a><a href="strength-v2.json" download>Historical v2 benchmark ↓</a><a href="solver-mathematics.md" target="_blank" rel="noopener noreferrer">Math & solver assumptions ↗</a><a href="implementation.md" target="_blank" rel="noopener noreferrer">Implementation & limitations ↗</a><a href="https://github.com/Cheekywnl/catan-ai-lab" target="_blank" rel="noopener noreferrer">Open GitHub ↗</a></div></section><div class="engine-footnote"><a href="engine-source.zip" download>Python engine source ↓</a><a href="THIRD_PARTY.md" target="_blank" rel="noopener noreferrer">Runtime & license notices ↗</a><span>Research sandbox · Replay exports include hidden information.</span></div>`;
