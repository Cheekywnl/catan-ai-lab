...
## Third cycle — 01:00 UTC

- Replayed the first 40 sorted games from the completed seeds 4500–4599 tournament to inspect public nonwin information. Across 3,828 completed turns, seven positions in four games had positive modeled VP probability for someone who just ended on nine public points. Captured seed 4500 / seat 3 / revision 439 as a regression: the old model returned 66.67% for Red, which the public history excludes. Hidden cards are used only in offline assertions, never as policy inputs.
- Engine 0.3.2 adds a joint VP allocation model constrained by latest completed turns, current purchases and observed game outcome. Old/new hand slots are conditioned together; non-VP types remain exchangeable. Search and forecasts sample these worlds directly. The reference policies and active tournament snapshots remain unchanged.
- Repeating the same audit with the new source gives zero contradictions and identical checksums for all 40 replays. `development-history-v032.json` records both source manifests, cases and replay checksums. These reused boards diagnose correctness; they are not held-out strength evidence.
- All 203 native tests pass. Six full-game support tests use seeds 6100–6105; reserve those as correctness boards, not fresh strength evaluation. Browser validation also checks the captured position and displayed VP range in the actual Python runtime.
- Browser validation:22 passed,2 intended mobile skips;6 content checks passed. Engine 0.3.1 budget 12 completed100 games with21 wins and0errors/truncations; matched Tactical control had26wins. Paired whole-board bootstrap difference is−5 percentage points, interval−13 to+3. This is not evidence of stronger search. Report `search12-v031-comparison.json`. Keep search experimental; the budget 48 arm continues unchanged.
- Next fixed protocol before outcomes: engine 0.3.2 budget 12 versus three strategic-v2 opponents,100games on fresh seeds 6200–6224/all4seats, tracking on,horizon 1600,max initial candidates4,maxactions4000. Four workers. Matched Tactical control on exactly those boards/settings,100games,two workers. Directories `../../work/experiments/search12-v032-6200` and `../../work/experiments/tactical-control-v032-6200`. These use the new sampler, so keep their evidence separate from0.3.1. Record runtime, sampler rejections and all failures; compare only after complete outcomes.

## Fourth cycle — 01:21 UTC

- Root repeatability audit used a frozen0.3.2 engine on planned tuning seeds 6400–6411. Eleven positions qualified;6408 ended before its scheduled threshold. All 11 audits completed without errors.132 repeated searches,3,960 search continuations,43 reference candidate actions and 2,752 independent reference continuations. Reported selected-score optimism averaged0.1186 at budget 12 and0.0509 at budget 48; recommendation modal shares57.6% and54.5%. This is descriptive tuning evidence under the continuation model, not a strength estimate. See `root-search-audit.md` and `root-variance-v032.json`. Source script and all packets/checkpoints remain in `../../work/experiments/root-variance-v032-6400`.
- A separate replay audit exposed dropped information: displayed Tactical suggestions used tracked resource cards, while autoplay/arena omitted them.43 of77 Monopoly positions across40 replays disagreed. The first captured exact-tracker position is seed4500 / revision177 / Blue: autoplay picked wood instead of the eight available wheat cards. The new policy-specific observation boundary removes all43 mismatches; replay checksums remain identical. See `policy-observation-v4.json` and `tests/fixtures/policy-observation-repro.json`.
- Engine 0.4's current `tactical` policy is Tactical v4 with tracked inputs; `tactical_v3` keeps the frozen input boundary. Core scoring modules remain unchanged.210 native tests pass. Root search's scoring already received beliefs and its candidate ranking is unchanged by this fix.
- Started fixed800-game validation at01:36UTC: seeds 6500–6699/all4seats, Tactical v4 against three `tactical_v3` opponents, tracking on, corrected rules, max4000 actions,4workers. Directory `../../work/experiments/tactical-v4-6500`, initialPID25364. Read `tactical-v4-protocol.md`; do not treat intermediate results as final. All prior active batches continue from their immutable snapshots.
- Completed at01:43UTC: **215/800 wins (26.875%)**, interval25.25–28.50%, zero errors/truncations. This supports a modest gain against frozen v3; retain the tracked-input fix as the current fast policy. See `strength-v4.json`. The original scoring weights and frozen v1/v2/v3 input modes remain available.
