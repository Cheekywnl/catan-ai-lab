...
const data = JSON.parse(
  await readFile(new URL("../dist/content.json", import.meta.url)),
);
...
test("all twelve chapters and all 37 evidence entries are present and linked", () => {
  ...
  for (const s of data.sources) {
    assert.ok(s.title && s.text && s.html);
    assert.equal(new URL(s.url).protocol, "https:");
    assert.equal(data.references[s.id], s.url);
  }
  for (const c of data.chapters) {
    assert.ok(c.text.length > 800);
    assert.doesNotMatch(c.html, /\[\d+\](?!<\/a>)/);
  }
...
test("download is an exact copy of the research source after newline normalization", async () => {
  const [source, download] = await Promise.all(
    ["../research/catan-ai-research.md", "../dist/catan-ai-research.md"].map(
...
test("all curated assessments and architecture nodes resolve to real evidence and chapters", () => {
  ...
  for (const item of [...projects, ...components])
    for (const id of item.refs) assert.ok(data.references[id]);
  for (const item of [...components, ...stages])
    assert.ok(data.chapters.some((c) => c.id === item.chapter));
});
  ...
  assert.deepEqual(theftOutcomes(2, 1), [
    {
      stolen: "ore",
      probability: 2 / 3,
      victim: { ore: 1, wheat: 1 },
      thief: { ore: 1, wheat: 0 },
    },
    {
      stolen: "wheat",
      probability: 1 / 3,
      victim: { ore: 2, wheat: 0 },
      thief: { ore: 0, wheat: 1 },
    },
  ]);
});
test("every allowed demo hand conserves each resource and total probability", () => {
  for (let ore = 0; ore <= 10; ore++)
    for (let wheat = 0; wheat <= 10; wheat++) {
      if (!ore && !wheat) continue;
      const outcomes = theftOutcomes(ore, wheat);
      assert.ok(
        Math.abs(outcomes.reduce((sum, o) => sum + o.probability, 0) - 1) <
          1e-12,
      );
      for (const o of outcomes) {
        assert.equal(o.victim.ore + o.thief.ore, ore);
        assert.equal(o.victim.wheat + o.thief.wheat, wheat);
        assert.equal(o.thief.ore + o.thief.wheat, 1);
        assert.ok(o.probability > 0);
      }
    }
});
test("empty hands, fractions, negative counts and nonnumbers are rejected", () => {
  for (const hand of [
    ...
    assert.throws(() => theftOutcomes(...hand), RangeError);
