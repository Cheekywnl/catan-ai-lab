...
test("overview, all chapters, source citations, and direct-link reloads", async ({
  page,
}) => {
  const errors = [];
  ...
  for (let id = 1; id <= 12; id++) {
    ...
    expect(await page.locator(".prose").innerText()).toHaveLengthGreaterThan(
      700,
    );
    await expect(
      page.locator('.chapter-nav a[aria-current="page"]'),
    ).toHaveCount(1);
  }
  ...
  await expect(
    page
      .locator(
        '.prose a[href="https://ojs.aaai.org/index.php/AIIDE/article/view/13014"]',
      )
      .first(),
  ).toBeVisible();
  await page.reload();
...
test("project and source filters provide real, recoverable results", async ({
  page,
}) => {
  ...
  await expect(page.locator(".source-entry")).toHaveCount(1);
  await expect(page.locator(".source-copy")).toContainText(
    "Community Guidelines",
  );
});

test("global search, query escaping, unknown routes, and report download", async ({
  page,
}) => {
  await page.goto("/");
  ...
  await page
    .getByLabel("Search all research")
  ...
  expect(await response.text()).toContain("Sources and evidence register");
  await page.getByRole("link", { name: "Download the full report" }).click();
  const download = await downloadPromise;
  expect(download.suggestedFilename()).toBe("catan-ai-research.md");
});

test("architecture selection and exact correlated belief calculations", async ({
  page,
}) => {
  await page.goto("/#architecture");
  await page.locator('a[href="#architecture/believe"]').click();
  await expect(page.locator("#component-detail h2")).toHaveText(
    "Keep possible hands, with probabilities.",
  );
...
test("all principal views meet automated WCAG checks and fit the viewport", async ({
  page,
}) => {
...
test("keyboard skip link and navigation preserve a usable focus target", async ({
  page,
}, testInfo) => {
  test.skip(
    testInfo.project.name === "mobile",
    "Hardware keyboard behavior is checked on desktop.",
  );
  await page.goto("/");
...
test("narrow screens and the layout equivalent of 200 percent desktop zoom reflow", async ({
  page,
}, testInfo) => {
  test.skip(
    testInfo.project.name === "mobile",
    "Additional CSS viewport widths are exercised once on desktop.",
  );
  for (const width of [320, 720]) {
    ...
    for (const route of [
      ...
      expect(sizes.scroll, `${route} at ${width}px`).toBeLessThanOrEqual(
        sizes.width + 1,
      );
    }
  }
...
expect.extend({
  toHaveLengthGreaterThan(received, minimum) {
    return {
      pass: received.length > minimum,
      message: () =>
        `Expected text length ${received.length} to exceed ${minimum}`,
    };
  },
});
